...
test('T026 [US5] click cell (energy=8, valence=9) shows positive mood label', async ({ page }) => {
  ...
  const display = page.locator('#selected-mood-display');
...
test('T027 [US5] select mood cell then reset clears selection', async ({ page }) => {
  await page.goto('/');
  await page.locator('.mood-cell[data-row="3"][data-col="7"]').click();
  await expect(page.locator('#selected-mood-display')).not.toHaveClass(/is-empty/);

  await page.locator('#reset-mood').click();
  await expect(page.locator('#selected-mood-display')).toHaveClass(/is-empty/);
...
test('T028 [US5] save entry with mood grid selection, reload, verify re-highlights', async ({ page }) => {
  ...
  await expect(page.locator('.mood-cell[data-row="5"][data-col="5"]')).toHaveClass(/is-selected/);
  await expect(page.locator('#selected-mood-display')).toContainText('E 5/10, V 6/10');
});
...
test('T029 [US5] clicking multiple cells keeps only last selection active', async ({ page }) => {
  ...
  await page.locator('.mood-cell[data-row="0"][data-col="0"]').click();
  await page.locator('.mood-cell[data-row="5"][data-col="5"]').click();
  await page.locator('.mood-cell[data-row="9"][data-col="9"]').click();

  // Only the last cell should be selected
  await expect(page.locator('.mood-cell.is-selected')).toHaveCount(1);
  await expect(page.locator('.mood-cell[data-row="9"][data-col="9"]')).toHaveClass(/is-selected/);
});
...
test('T030 [US5] corner cells render correct boundary labels', async ({ page }) => {
  ...
  await page.locator('.mood-cell[data-row="9"][data-col="0"]').click();
  await expect(page.locator('#selected-mood-display')).toContainText('E 1/10, V 1/10');

  // (1,10) = row=9, col=9 — Low energy, Positive
  await page.locator('.mood-cell[data-row="9"][data-col="9"]').click();
  await expect(page.locator('#selected-mood-display')).toContainText('E 1/10, V 10/10');

  // (10,1) = row=0, col=0 — High energy, Negative
  await page.locator('.mood-cell[data-row="0"][data-col="0"]').click();
  await expect(page.locator('#selected-mood-display')).toContainText('E 10/10, V 1/10');

  // (10,10) = row=0, col=9 — High energy, Positive
  await page.locator('.mood-cell[data-row="0"][data-col="9"]').click();
  await expect(page.locator('#selected-mood-display')).toContainText('E 10/10, V 10/10');
});
